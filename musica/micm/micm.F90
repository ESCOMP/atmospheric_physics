...
contains
   ...
   subroutine micm_init(config_path, iulog, errcode, errmsg)
       ...
       if (errcode /= 0) then
           !TODO(jiwon) - which file?
           errmsg = "INIT MICM: FATAL: Failed in creating MICM solver because parsing &
                   configuration files failed. Please look over at the other file."  
       else 
   ...
   subroutine micm_run(time_step, temperature, pressure, dry_air_density, constituent_props, &
       ...
       do i_elem = 1, num_constituents
           ...
           if (errcode /= 0) then
               write(iulog,*) "   [RUN MICM]: FATAL: Unable to get molar mass at n-th element: ", &
       ...
       do i_elem = 1, num_constituents
           if (molar_mass_arr(i_elem) == 0) then
               write(iulog,*) "   [RUN MICM]: FATAL: molar mass cannot be zero"
               errcode = 1
               stop 3
           end if
       end do
