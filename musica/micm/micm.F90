module micm
 ...
 contains
   ...
   subroutine micm_run(temperature, pressure, time_step, concentrations, num_concentrations, iulog, errcode, errmsg)
      ...
      ! local variables
      real(c_double), dimension(shape(temperature))      :: c_temperature
      ...
      horizontal_loop_extent = size(concentrations, 1)
      vertical_layer_dimension  = size(concentrations, 2)
      ...
      do i_column = 1, horizontal_loop_extent
         do i_layer = 1, vertical_layer_dimension
            call micm_ptr%solve(c_temperature(i_column, i_layer), c_pressure(i_column, i_layer), c_time_step, &
                                c_concentrations(i_column, i_layer, :), num_concentrations)
         end do
      end do 
