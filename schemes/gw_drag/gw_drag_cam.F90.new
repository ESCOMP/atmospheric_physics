...
!jt  use gw_drag,        only: gw_drag_init, gw_drag_run
...
subroutine gw_tend(state, pbuf, dt, ptend, cam_in, flx_heat)
  ...
  use gw_oro,          only: gw_oro_run
  use gw_front,        only: gw_cm_run
  use gw_convect,      only: gw_beres_run
...
!!$  Following in gw_drag_timestep_init
...
!!$  end if
  ...
  if (use_gw_movmtn_pbl) then
     ...
     call gw_movmtn_run( ncol, lchnk, band_movmtn , movmtn_desc, &
          state%u, state%v, ttend_dp(:ncol,:), ttend_clubb(:ncol,:), xpwp_clubb(:ncol,:)  , &
          state%zm, alpha_gw_movmtn, src_level, tend_level, &
          tau, ubm, ubi, xv, yv, &
          phase_speeds, hdepth, ptend%s, ptend%u, ptend%v, ptend%t, ptend%q, &
          p%ifc, p%rdst, p%del, p%rdel, dt, &
          state%t, vramp,    &
          state%piln, rhoi,       nm,   ni, &
          effgw,   phase_speeds,       kvtt, state%q,  dse,  utgw,  vtgw, &
          ttgw, qtgw, egwdffi,  gwut, dttdf, dttke,            &
          lapply_effgw_in=gw_apply_tndmax, scheme_name, errmsg, errflg )
