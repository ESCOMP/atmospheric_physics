...
module modal_aero_rename
  ...
  real (kind_phys) :: dp_belowcut(maxpair_renamexf)
  real (kind_phys) :: dp_cut(maxpair_renamexf)
  real (kind_phys) :: dp_xferall_thresh(maxpair_renamexf)
  real (kind_phys) :: dp_xfernone_threshaa(maxpair_renamexf)
  real (kind_phys), allocatable :: dryvol_smallest(:)
  real (kind_phys), allocatable :: factoraa(:)
  real (kind_phys), allocatable :: factoryy(:)
  real (kind_phys) :: lndp_cut(maxpair_renamexf)
  real (kind_phys) :: factor_3alnsg2(maxpair_renamexf)
...
contains
  ...
  subroutine modal_aero_rename_init(                             &
    ...
    real(kind_phys), intent(in) :: dgnum_amode(:)
    real(kind_phys), intent(in) :: dgnumhi_amode(:)
    real(kind_phys), intent(in) :: dgnumlo_amode(:)
    real(kind_phys), intent(in) :: voltonumblo_amode(:)
    real(kind_phys), intent(in) :: voltonumbhi_amode(:)
...
!   and number = 1e-5 #/mg-air ~= 1e-5 #/cm3-air
      ...
      factor_3alnsg2(ipair) = 3.0_kind_phys * (alnsg_amode(mfrm)**2)
      ...
      if (((mfrm == modeptr_accum) .and. (mtoo == modeptr_coarse)).or.&
               ...
               dp_cut(ipair)               = 4.4e-7_kind_phys
               dp_xfernone_threshaa(ipair) = 1.6e-7_kind_phys
               dp_xferall_thresh(ipair)    = 4.7e-7_kind_phys
            else if (((mfrm == modeptr_coarse) .and. (mtoo == modeptr_accum)).or.&
                     ((mfrm == modeptr_stracoar) .and. (mtoo == modeptr_accum))) then
               dp_cut(ipair)               = 4.4e-7_kind_phys
               dp_xfernone_threshaa(ipair) = 4.4e-7_kind_phys
...
9350  format(5x, 'no corresponding activated species')
  ...
  subroutine modal_aero_rename_run(                      &
    ...
    real(kind_phys), intent(in)    :: q(:,:,:)             ! tracer mixing ratio array (mol/mol-air or #/mol-air)
    real(kind_phys), intent(in)    :: qqcw(:,:,:)          ! like q but for cloud-borne species
    real(kind_phys), intent(inout) :: dqdt(:,:,:)          ! TMR tendency array (renaming tendencies added on)
    real(kind_phys), intent(inout) :: dqqcwdt(:,:,:)
    real(kind_phys), intent(in)    :: dqdt_other(:,:,:)    ! tendencies for "other" continuous growth process
    real(kind_phys), intent(in)    :: dqqcwdt_other(:,:,:)
  ...
  subroutine modal_aero_rename_no_acc_crs_sub(                       &
   ...
   real(kind_phys), intent(in)    :: alnsg_amode(:)       ! ln(geometric std dev) of each mode
   real(kind_phys), intent(in)    :: voltonumblo_amode(:) ! volume-to-number ratio, low limit
   real(kind_phys), intent(in)    :: voltonumbhi_amode(:) ! volume-to-number ratio, high limit
   real(kind_phys), intent(in)    :: dgnum_amode(:)       ! nominal geometric mean diameter
   integer,  intent(in)    :: nspec_amode(:)       ! number of species in each mode
   real(kind_phys), intent(in)    :: specmw_amode(:,:)    ! species molecular weight
   real(kind_phys), intent(in)    :: specdens_amode(:,:)  ! species density
...
! local variables
   ...
   real (kind_phys) :: deldryvol_a(ncol,pver,ntot_amode)
   real (kind_phys) :: deldryvol_c(ncol,pver,ntot_amode)
   real (kind_phys) :: deltatinv
   real (kind_phys) :: dp_belowcut(maxpair_renamexf)
   real (kind_phys) :: dp_cut(maxpair_renamexf)
   real (kind_phys) :: dgn_aftr, dgn_xfer
   real (kind_phys) :: dgn_t_new, dgn_t_old
   real (kind_phys) :: dryvol_t_del, dryvol_t_new
   real (kind_phys) :: dryvol_t_old, dryvol_t_oldbnd
   real (kind_phys) :: dryvol_a(ncol,pver,ntot_amode)
   real (kind_phys) :: dryvol_c(ncol,pver,ntot_amode)
   real (kind_phys) :: dryvol_smallest(ntot_amode)
   real (kind_phys) :: dum
   real (kind_phys) :: dum3alnsg2(maxpair_renamexf)
   real (kind_phys) :: dum_m2v, dum_m2vdt
   real (kind_phys) :: factoraa(ntot_amode)
   real (kind_phys) :: factoryy(ntot_amode)
   real (kind_phys) :: frelax
   real (kind_phys) :: lndp_cut(maxpair_renamexf)
   real (kind_phys) :: lndgn_new, lndgn_old
   real (kind_phys) :: lndgv_new, lndgv_old
   real (kind_phys) :: num_t_old, num_t_oldbnd
   real (kind_phys) :: onethird
   real (kind_phys) :: pdel_fac
   real (kind_phys) :: tailfr_volnew, tailfr_volold
   real (kind_phys) :: tailfr_numnew, tailfr_numold
   real (kind_phys) :: v2nhirlx(ntot_amode), v2nlorlx(ntot_amode)
   real (kind_phys) :: xfercoef, xfertend
   real (kind_phys) :: xferfrac_vol, xferfrac_num, xferfrac_max
...
! !PARAMETERS:
   ...
   real(kind_phys), intent(out)   :: qsrflx(:,:,:)
                              ! process-specific column tracer tendencies
   real(kind_phys), intent(out)   :: qqcwsrflx(:,:,:)
   real(kind_phys), intent(out)   :: dqdt_rnpos(:,:,:)
...
! local variables
   ...
   real (kind_phys) :: deldryvol_a(ncol,pver)
   real (kind_phys) :: deldryvol_c(ncol,pver)
   real (kind_phys) :: deltatinv
   real (kind_phys) :: dgn_aftr, dgn_xfer
   real (kind_phys) :: dgn_t_new, dgn_t_old, dgn_t_oldb
   real (kind_phys) :: dryvol_t_del, dryvol_t_new, dryvol_t_new_xfab
   real (kind_phys) :: dryvol_t_old, dryvol_t_oldb, dryvol_t_oldbnd
   real (kind_phys) :: dryvol_a(ncol,pver)
   real (kind_phys) :: dryvol_c(ncol,pver)
   real (kind_phys) :: dryvol_a_xfab(ncol,pver)
   real (kind_phys) :: dryvol_c_xfab(ncol,pver)
   real (kind_phys) :: dryvol_xferamt
   real (kind_phys) :: lndgn_new, lndgn_old
   real (kind_phys) :: lndgv_new, lndgv_old
   real (kind_phys) :: num_t_old, num_t_oldbnd
   real (kind_phys) :: onethird
   ...
   real (kind_phys) :: tailfr_volnew, tailfr_volold
   real (kind_phys) :: tailfr_numnew, tailfr_numold
   real (kind_phys) :: tmpa, tmpf
   real (kind_phys) :: tmp_m2v, tmp_m2vdt
   real (kind_phys) :: xfercoef, xfertend
   real (kind_phys) :: xferfrac_vol, xferfrac_num, xferfrac_max
...
! using the incoming dqdt*deltat
!
  dryvol_a(:,:) = 0.0_kind_phys
  dryvol_c(:,:) = 0.0_kind_phys
  deldryvol_a(:,:) = 0.0_kind_phys
  deldryvol_c(:,:) = 0.0_kind_phys
  if (ixferable_all_renamexf(ipair) <= 0) then
      dryvol_a_xfab(:,:) = 0.0_kind_phys
      dryvol_c_xfab(:,:) = 0.0_kind_phys
  end if
...
!   no need to compute old fraction of number and mass in the tail
  tailfr_numold = 0.0_kind_phys
  tailfr_volold = 0.0_kind_phys
