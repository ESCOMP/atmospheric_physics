! Portable science kernel for Abdul-Razzak & Ghan aerosol activation.
...
module aero_activate
  ...
  public :: aero_activate_init
  public :: activate_aerosol
  ...
  ! surface tension of water w.r.t. air [N m-1]
  real(kind_phys), parameter :: surften = 0.076_kind_phys

  ! derived constants (set by aero_activate_init based on host physical constants)
  real(kind_phys) :: aten
  real(kind_phys) :: sqpi
...
contains
  ...
  subroutine activate_aerosol(wbar, sigw, wdiab, wminf, wmaxf, tair, rhoair, &
    ...
    ! droplet and rain size distr factor in the smax calculation used when in_cloud=.true.
    real(kind_phys), optional, intent(in) :: smax_f
