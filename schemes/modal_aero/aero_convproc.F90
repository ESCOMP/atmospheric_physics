...
module aero_convproc
  ...
  real(kind_phys), parameter, public :: method2_activate_smaxmax = 0.003_kind_phys
  ...
  real(kind_phys), parameter, public :: factor_reduce_actfrac = 0.5_kind_phys
...
contains
  ...
  subroutine aero_convproc_run(aero_props, convtype, lchnk, dt, &
    ...
    real(kind_phys), intent(in) :: rhoh2o           ! density of liquid water (STP) (kg/m3)
    real(kind_phys), intent(in) :: rh2o             ! gas constant for water vapor (J/K/kg)
    ...
    real(kind_phys), intent(in) :: q(:, :, :)          ! Tracer array including moisture
...
! *** note4 -- for il1g <= i <= il2g,  icol = ideep(i) is the "normal" chunk column index
    ...
    real(kind_phys), intent(in) :: rprd(:, :)         ! Convective precipitation formation rate
    ...
    real(kind_phys), intent(out) :: xx_mfup_max(:)
    real(kind_phys), intent(out) :: xx_wcldbase(:)
    real(kind_phys), intent(out) :: xx_kcldbase(:)
    real(kind_phys), intent(inout) :: dcondt_resusp3d(:, :, :)
    real(kind_phys), intent(out) :: conu2(:, :, :, :)   ! updraft interface TMR diagnostic (WETC/CONU history)
    real(kind_phys), intent(out) :: dcondt2(:, :, :, :) ! wet-deposition TMR tendency diagnostic (WETC/CONU history)
...
!
    ...
    hund_ovr_g = 100.0_kind_phys/gravit
...
! mbsth is the threshold below which we treat the mass fluxes as zero (in mb/s)
    ...
    xx_wcldbase(:) = 0.0_kind_phys
    xx_kcldbase(:) = 0.0_kind_phys

    wup(:) = 0.0_kind_phys

    dcondt2 = 0.0_kind_phys
    conu2 = 0.0_kind_phys
...
! resuspension from evaporating precip can occur at k > kbot when kbot < pver
      ...
      do k = ktop + 1, kbot
        mu_i(k) = mu_x(k)
        if (mu_i(k) <= mbsth) mu_i(k) = 0.0_kind_phys
        md_i(k) = md_x(k)
        if (md_i(k) >= -mbsth) md_i(k) = 0.0_kind_phys
      end do
      mu_i(ktop) = 0.0_kind_phys
      md_i(ktop) = 0.0_kind_phys
      mu_i(kbot + 1) = 0.0_kind_phys
      md_i(kbot + 1) = 0.0_kind_phys
...
!   (1st to get non-adjusted activation amount, 2nd to apply reduction factor)
      ...
      jtsub_loop_main_aa: &
        ...
        ipass_calc_updraft_loop: &
          ...
          qsrflx_i(:, :) = 0.0_kind_phys
          dqdt_i(:, :) = 0.0_kind_phys

          const = 0.0_kind_phys ! zero cloud-phase species
          chat = 0.0_kind_phys ! zero cloud-phase species
          conu = 0.0_kind_phys
          cond = 0.0_kind_phys

          dcondt = 0.0_kind_phys
          dcondt_resusp = 0.0_kind_phys
          dcondt_wetdep = 0.0_kind_phys
          dcondt_prevap = 0.0_kind_phys
          dconudt_aqchem = 0.0_kind_phys
          dconudt_wetdep = 0.0_kind_phys

! only initialize the activation tendency on ipass=1
          if (ipass_calc_updraft == 1) dconudt_activa = 0.0_kind_phys
          ...
          do m = 1, aero_props%nbins()
            do l = 0, aero_props%nmasses(m)
              ...
              do k = 1, pver
                ...
                if (iconvtype /= 1) then
                  chat(1, mm, k) = 0.5_kind_phys*(const(1, mm, k) + const(1, mm, km1))
                else if (cdifr > 1.E-6_kind_phys) then
                  cabv = max(const(1, mm, km1), maxc*1.e-12_kind_phys)
                  cbel = max(const(1, mm, k), maxc*1.e-12_kind_phys)
...
! mu_p_eudp(k) = updraft massflux at k, without detrainment between kp1,k
            ...
            fa_u(k) = 0.0_kind_phys !BSINGH(10/15/2014): Initialized so that it has a value if the following "if" check yeilds .false.
...
! shallow - wup = (mup in kg/m2/s) / [rhoair * (updraft area)]
                wup(k) = (mu_i(kp1) + mu_i(k))*0.5_kind_phys*hund_ovr_g &
                         /(rhoair_i(k)*(cldfrac_i(k)*0.5_kind_phys))
              else
...
!    when kactcnt>1 apply activatation to the amount entrained at this level
                if ((icwmr(icol, k) > clw_cut) .and. (fracice(icol, k) < 1.0_kind_phys)) then
                  kactcnt = kactcnt + 1

                  if ((kactcnt == 1) .or. (f_ent > 0.0_kind_phys)) then
                    kactcntb = kactcntb + 1
                  end if
                  ...
                  if (kactcnt == 1) then
                    ...
                    xx_wcldbase(icol) = (mu_i(kp1) + mu_i(k))*0.5_kind_phys*hund_ovr_g &
                                        /(rhoair_i(k)*(cldfrac_i(k)*0.5_kind_phys))
                    xx_kcldbase(icol) = k
...
!
              cdt(k) = 0.0_kind_phys
              if ((icwmr(icol, k) > clw_cut) .and. (rprd(icol, k) > 0.0_kind_phys)) then
!              if (iconvtype == 1) then
                tmpf = 0.5_kind_phys*cldfrac_i(k)
...
!              end if
              ...
              if (cdt(k) > 0.0_kind_phys) then
                expcdtm1 = exp(-cdt(k)) - 1.0_kind_phys
...
!        must be modified to include source and sink terms
          sumflux = 0.0_kind_phys
          sumflux2 = 0.0_kind_phys
          sumsrce = 0.0_kind_phys
          sumchng = 0.0_kind_phys
          sumchng3 = 0.0_kind_phys
          sumwetdep = 0.0_kind_phys
          sumprevap = 0.0_kind_phys

          maxflux = 0.0_kind_phys
          maxflux2 = 0.0_kind_phys
          maxresusp = 0.0_kind_phys
          maxsrce = 0.0_kind_phys
          maxprevap = 0.0_kind_phys
...
! (note:  TMR = tracer mixing ratio)
    ...
    real(kind_phys), intent(in)    :: rprd(:, :)  ! conv precip production  rate (gathered)
    real(kind_phys), intent(in)    :: evapc(:, :)  ! conv precip evaporation rate (gathered)
    real(kind_phys), intent(in)    :: dp_i(pver) ! pressure thickness of level (in mb)
...
! local variables
    integer  :: k, l, m, mm, n
    real(kind_phys) :: del_pr_flux_prod      ! change to precip flux from production  [(kg/kg/s)*mb]
    real(kind_phys) :: del_pr_flux_evap      ! change to precip flux from evaporation [(kg/kg/s)*mb]
    real(kind_phys) :: del_wd_flux_evap      ! change to wet deposition flux from evaporation [(kg/kg/s)*mb]
    real(kind_phys) :: fdel_pr_flux_evap     ! fractional change to precip flux from evaporation
    real(kind_phys) :: pr_flux               ! precip flux at base of current layer [(kg/kg/s)*mb]
    real(kind_phys) :: pr_flux_old
    real(kind_phys) :: tmpdp                 ! delta-pressure (mb)
    real(kind_phys) :: wd_flux(2, ncnstaer)   ! tracer wet deposition flux at base of current layer [(kg/kg/s)*mb]
!-----------------------------------------------------------------------

    pr_flux = 0.0_kind_phys
    wd_flux = 0.0_kind_phys
      ...
      do m = 1, aero_props%nbins()
        do l = 0, aero_props%nmasses(m)
          ...
          do n = 1, 2
            ...
            wd_flux(n, mm) = wd_flux(n, mm) + tmpdp*max(0.0_kind_phys, -dcondt_wetdep(n, mm, k))
...
! arguments  (note:  TMR = tracer mixing ratio)
    ...
    real(kind_phys), intent(inout) :: conu(2, ncnstaer)
    ! conent = TMRs in the entrained air at this level
    real(kind_phys), intent(in)    :: conent(2, ncnstaer)
    real(kind_phys), intent(inout) :: dconudt(2, ncnstaer) ! TMR tendencies due to activation

    real(kind_phys), intent(in)    :: f_ent  ! fraction of updraft massflux that was
    ! entrained across this layer == eudp/mu_p_eudp
    real(kind_phys), intent(in)    :: dt_u   ! lagrangian transport time (s) in the
    ! updraft at current level
    real(kind_phys), intent(in)    :: wup    ! mean updraft vertical velocity (m/s)
    ! at current level updraft

    real(kind_phys), intent(in)    :: tair   ! Temperature in Kelvin
...
! local variables
    ...
    real(kind_phys) :: dt_u_inv         ! 1.0/dt_u
    real(kind_phys) :: fluxm(nbins)     ! to understand this, see subr activate_aerosol
    real(kind_phys) :: fluxn(nbins)     ! to understand this, see subr activate_aerosol
    real(kind_phys) :: flux_fullact     ! to understand this, see subr activate_aerosol
    real(kind_phys) :: fm(nbins)        ! mass fraction of aerosols activated
    real(kind_phys) :: fn(nbins)        ! number fraction of aerosols activated
    real(kind_phys) :: hygro(nbins)     ! current hygroscopicity for int+act
    real(kind_phys) :: naerosol(nbins)  ! interstitial+activated number conc (#/m3)
    real(kind_phys) :: sigw             ! standard deviation of updraft velocity (cm/s)
    real(kind_phys) :: tmp_fact         ! working variable
    real(kind_phys) :: vaerosol(nbins)  ! int+act volume (m3/m3)
    real(kind_phys) :: wbar             ! mean updraft velocity (cm/s)
    real(kind_phys) :: wdiab            ! diabatic vertical velocity (cm/s)
    real(kind_phys) :: wminf, wmaxf     ! limits for integration over updraft spectrum (cm/s)

    real(kind_phys) :: spec_hygro
    real(kind_phys) :: spec_dens
    character(len=32) :: spec_type

    real(kind_phys) :: tmpa, tmpb, tmpc ! working variable
...
! compute a (or a+cw) volume and hygroscopicity
      ...
      if (tmpa < 1.0e-35_kind_phys) then
        hygro(m) = 0.2_kind_phys
      else
        hygro(m) = tmpb/tmpa
      end if
...
! local variables
    ...
    real(kind_phys) :: naerosol(nbins)  ! interstitial+activated number conc (#/m3)
    real(kind_phys) :: sigw             ! standard deviation of updraft velocity (cm/s)
    real(kind_phys) :: smax_prescribed  ! prescribed supersaturation for secondary activation (0-1 fraction)
    real(kind_phys) :: tmp_fact         ! working variable
    real(kind_phys) :: vaerosol(nbins)  ! int+act volume (m3/m3)
    real(kind_phys) :: wbar             ! mean updraft velocity (cm/s)
    real(kind_phys) :: wdiab            ! diabatic vertical velocity (cm/s)
...
! compute a (or a+cw) volume and hygroscopicity
      ...
      do l = 1, aero_props%nspecies(m)
        ...
        if (spec_type == 'p-organic' .and. convproc_pom_spechygro > 0._kind_phys) then
...
! (note:  TMR = tracer mixing ratio)
    ...
    real(kind_phys), intent(inout) :: dcondt(2, ncnstaer, pver)
    ! overall TMR tendency from convection
    real(kind_phys), intent(inout) :: dcondt_resusp(2, ncnstaer, pver)
