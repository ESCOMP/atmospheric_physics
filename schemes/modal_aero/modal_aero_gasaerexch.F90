...
module modal_aero_gasaerexch
  ...
  real(kind_phys), parameter, public :: n_so4_monolayers_pcage = 8.0_kind_phys
  ...
  real(kind_phys), protected, allocatable, public :: soa_equivso4_factor(:)
  ...
  ! Mode metadata
  real(kind_phys), allocatable :: alnsg_amode_m(:), sigmag_amode_m(:)
  real(kind_phys), allocatable :: specmw_amode_m(:, :), specdens_amode_m(:, :)
  ...
  real(kind_phys) :: fac_m2v_nh4, fac_m2v_so4
  real(kind_phys), allocatable :: fac_m2v_soa(:)

  real(kind_phys), allocatable :: fac_m2v_pcarbon(:)

  ! SOA/POA molecular weights from host model
  real(kind_phys), allocatable :: mw_soa_host_m(:), mw_poa_host_m(:)

  ! Host-provided physical constants:
  real(kind_phys) :: rair_m, mwdry_m
  real(kind_phys) :: rgas_m
...
contains
  ...
  subroutine modal_aero_gasaerexch_init( &
    ...
    real(kind_phys), intent(in) :: alnsg_amode(:)
    real(kind_phys), intent(in) :: sigmag_amode(:)
    real(kind_phys), intent(in) :: specmw_amode(:, :)
    real(kind_phys), intent(in) :: specdens_amode(:, :)
    real(kind_phys), intent(in) :: spechygro(:, :)
    ...
    real(kind_phys), intent(in) :: mw_soa_host(:)
    real(kind_phys), intent(in) :: mw_poa_host(:)
    real(kind_phys), intent(in) :: rair               ! dry-air gas constant from host (J/K/kg)
    real(kind_phys), intent(in) :: mwdry              ! dry-air molecular weight from host (kg/kmol)
    real(kind_phys), intent(in) :: r_universal        ! universal gas constant from host (J/K/kmol)
...
!-----------------------------------------------------------------------
    ...
    rair_m = rair
    mwdry_m = mwdry
    rgas_m = r_universal*1.0e-3_kind_phys  ! J/K/kmol -> J/K/mol
    ...
    !    soa_equivso4_factor(:)
    soa_equivso4_factor = 0.0_kind_phys
    if (modefrm_pcage > 0) then
      n = modeptr_accum
      l2 = -1
...
!! \htmlinclude modal_aero_gasaerexch_run.html
  subroutine modal_aero_gasaerexch_run( &
    ...
    real(kind_phys), intent(in)    :: pmid(:, :)            ! (ncol,pver) pressure [Pa]
    real(kind_phys), intent(in)    :: pdel(:, :)            ! (ncol,pver) pressure thickness [Pa]
    real(kind_phys), intent(in)    :: gravit               ! gravitational acceleration [m s-2]
    real(kind_phys), intent(in)    :: dgncur_a(:, :, :)      ! (ncol,pver,ntot_amode) dry diameter
    real(kind_phys), intent(in)    :: dgncur_awet(:, :, :)   ! (ncol,pver,ntot_amode) wet diameter
    logical, intent(in)    :: use_sulfeq           ! whether to use strat equilibrium
    real(kind_phys), intent(in)    :: sulfeq(:, :, :)        ! (ncol,pver,ntot_amode) sulfeq values
    integer, intent(in)    :: num_q                ! # of species in vmr array (= gas_pcnst)
    real(kind_phys), intent(in)    :: q(:, :, :)             ! (ncol,pver,num_q) tracer VMR
    ! *** MUST BE  #/kmol-air for number
    ! *** MUST BE mol/mol-air for mass
    real(kind_phys), intent(out)   :: dqdt(:, :, :)          ! (ncol,pver,num_q) tendencies
    logical, intent(out)   :: dotend(:)            ! (num_q) which species have tendencies
    real(kind_phys), intent(out)   :: qsrflx_gaexch(:, :)   ! (ncol,num_q) column-integrated gas-aerosol
    ...
    real(kind_phys) :: deltatxx
    real(kind_phys) :: dqdt_nh4(ntot_amode_m), dqdt_so4(ntot_amode_m)
    real(kind_phys) :: dqdt_soa(ntot_amode_m, nsoa_m)
    real(kind_phys) :: dqdt_soag(nsoa_m)
    real(kind_phys) :: fac_volsfc_pcarbon
    real(kind_phys) :: fgain_nh4(ntot_amode_m), fgain_so4(ntot_amode_m)
    real(kind_phys) :: fgain_soa(ntot_amode_m, nsoa_m)
    real(kind_phys)  :: mw_poa_host(npoa_m)          ! molec wght of poa used in host code
    real(kind_phys)  :: mw_soa_host(nsoa_m)          ! molec wght of poa used in host code
    real(kind_phys) :: qmax_nh4, qnew_nh4, qnew_so4
    real(kind_phys) :: qold_nh4(ntot_amode_m), qold_so4(ntot_amode_m)
    real(kind_phys) :: qold_poa(ntot_amode_m, npoa_m)
    real(kind_phys) :: qold_soa(ntot_amode_m, nsoa_m)
    real(kind_phys) :: qold_soag(nsoa_m)
    real(kind_phys) :: sum_dqdt_msa, sum_dqdt_so4
    real(kind_phys) :: sum_dqdt_soa(nsoa_m)
    real(kind_phys) :: sum_dqdt_nh4, sum_dqdt_nh4_b
    real(kind_phys) :: sum_uprt_nh4, sum_uprt_so4
    real(kind_phys) :: sum_uprt_soa(nsoa_m)
    real(kind_phys) :: pdel_fac        ! pdel/gravit (kg/m2 per Pa), for column-integrated diagnostics
    real(kind_phys) :: tmp1, tmp2, tmpa
    real(kind_phys) :: tmp_kxt, tmp_pxt
    real(kind_phys) :: tmp_so4a_bgn, tmp_so4a_end
    real(kind_phys) :: tmp_so4g_avg, tmp_so4g_bgn, tmp_so4g_equ
    real(kind_phys) :: uptkrate(ntot_amode_m, ncol, pver)
    real(kind_phys) :: uptkratebb(ntot_amode_m)
    real(kind_phys) :: uptkrate_soa(ntot_amode_m, nsoa_m)
    ! gas-to-aerosol mass transfer rates (1/s)
    real(kind_phys) :: vol_core, vol_shell
    real(kind_phys) :: xferfrac_pcage, xferfrac_max
    real(kind_phys) :: xferrate
...
! use this for tendency calcs to avoid generating very small negative values
    deltatxx = deltat*(1.0_kind_phys + 1.0e-15_kind_phys)
...
!   fgain_nh4(n) = fraction of total  nh3  uptake going to mode n
        sum_uprt_so4 = 0.0_kind_phys
        sum_uprt_nh4 = 0.0_kind_phys
        sum_uprt_soa = 0.0_kind_phys
        do n = 1, ntot_amode_m
          ...
          if (ido_so4a(n) > 0) then
            ...
            if (ido_so4a(n) == 1) then
              ...
              qold_so4(n) = 0.0_kind_phys
            end if
          else
            fgain_so4(n) = 0.0_kind_phys
            qold_so4(n) = 0.0_kind_phys
          end if
          ...
          if (ido_nh4a(n) > 0) then
            ...
            fgain_nh4(n) = uptkratebb(n)*2.08_kind_phys
            sum_uprt_nh4 = sum_uprt_nh4 + fgain_nh4(n)
            if (ido_nh4a(n) == 1) then
              qold_nh4(n) = q(i, k, idx_nh4_a_q(n))
            else
              qold_nh4(n) = 0.0_kind_phys
            end if
          else
            fgain_nh4(n) = 0.0_kind_phys
            qold_nh4(n) = 0.0_kind_phys
          end if
          ...
          do jsoa = 1, nsoa_m
            if (ido_soaa(n, jsoa) > 0) then
              ...
              ! (differences in fuch-sutugin and accom coef ignored)
              fgain_soa(n, jsoa) = uptkratebb(n)*0.81_kind_phys
              ...
              if (ido_soaa(n, jsoa) == 1) then
                ...
                qold_soa(n, jsoa) = 0.0_kind_phys
              end if
            else
              fgain_soa(n, jsoa) = 0.0_kind_phys
              qold_soa(n, jsoa) = 0.0_kind_phys
            end if
          ...
          if (itmpa == 0) qold_poa(n, :) = 0.0_kind_phys

        end do ! n

        if (sum_uprt_so4 > 0.0_kind_phys) then
          do n = 1, ntot_amode_m
            fgain_so4(n) = fgain_so4(n)/sum_uprt_so4
          end do
        end if
!       at this point (sum_uprt_so4 <= 0.0) only when all the fgain_so4 are zero
        if (sum_uprt_nh4 > 0.0_kind_phys) then
          do n = 1, ntot_amode_m
        ...
        do jsoa = 1, nsoa_m
          if (sum_uprt_soa(jsoa) > 0.0_kind_phys) then
            do n = 1, ntot_amode_m
...
!   uptake amount (fraction of gas uptaken) over deltat
        avg_uprt_so4 = (1.0_kind_phys - exp(-deltatxx*sum_uprt_so4))/deltatxx
        avg_uprt_nh4 = (1.0_kind_phys - exp(-deltatxx*sum_uprt_nh4))/deltatxx

        do jsoa = 1, nsoa_m
          avg_uprt_soa(jsoa) = (1.0_kind_phys - exp(-deltatxx*sum_uprt_soa(jsoa)))/deltatxx
        end do
...
!   sum_dqdt_soa = soa_a tendency from soa   gas uptake (mol/mol/s)
        ...
        if (use_sulfeq .and. (k <= troplev(i))) then
          ...
          if (tmp_kxt >= 1.0e-5_kind_phys) then
            ...
            tmp_so4g_avg = tmp_so4g_equ + (tmp_so4g_bgn - tmp_so4g_equ)*(1.0_kind_phys - exp(-tmp_kxt))/tmp_kxt
          else
            ! first order approx for tmp_kxt small
            tmp_so4g_avg = tmp_so4g_bgn*(1.0_kind_phys - 0.5_kind_phys*tmp_kxt) + 0.5_kind_phys*tmp_pxt
          end if
          sum_dqdt_so4 = 0.0_kind_phys
          do n = 1, ntot_amode_m
            if (ido_so4a(n) <= 0) cycle
        ...
        if (do_nh4g) then
          do n = 1, ntot_amode_m
            ...
            qmax_nh4 = 2.0_kind_phys*qnew_so4
...
!
  ...
  subroutine gas_aer_uptkrates(ncol, pver, top_lev, &
    ...
    real(kind_phys), intent(in) :: q(:, :, :)             ! (ncol,pver,num_q) Tracer array (mol,#/mol-air)
    real(kind_phys), intent(in) :: t(:, :)               ! (ncol,pver) Temperature in Kelvin
    real(kind_phys), intent(in) :: pmid(:, :)            ! (ncol,pver) Air pressure in Pa
    real(kind_phys), intent(in) :: dgncur_awet(:, :, :)   ! (ncol,pver,ntot_amode_m)

    real(kind_phys), intent(out) :: uptkrate(:, :, :)     ! (ntot_amode_m,ncol,pver)
    ! gas-to-aerosol mass transfer rates (1/s)
...
! local
    ...
    real(kind_phys), parameter :: tworootpi = 3.5449077_kind_phys
    real(kind_phys), parameter :: root2 = 1.4142135_kind_phys
    real(kind_phys), parameter :: beta = 2.0_kind_phys

    real(kind_phys) :: aircon
    real(kind_phys) :: const
    real(kind_phys) :: dp
    real(kind_phys) :: gasdiffus, gasspeed
    real(kind_phys) :: freepathx2, fuchs_sutugin
    real(kind_phys) :: knudsen
    real(kind_phys) :: lndp, lndpgn, lnsg
    real(kind_phys) :: num_a
    real(kind_phys) :: rhoair
    real(kind_phys) :: sumghq
    real(kind_phys), save :: xghq(nghq), wghq(nghq) ! quadrature abscissae and weights
...
!-----------------------------------------------------------------------
    ...
    real(kind_phys), intent(in)  :: mw_poa_host(ntot_poaspec)  ! molec wght of poa used in host code
    real(kind_phys), intent(in)  :: mw_soa_host(ntot_soaspec)  ! molec wght of poa used in host code
    real(kind_phys), intent(in)  :: g_soa_in(ntot_soaspec)               ! initial soa gas mixrat (mol/mol at host mw)
    real(kind_phys), intent(in)  :: a_soa_in(ntot_amode, ntot_soaspec)    ! initial soa aerosol mixrat (mol/mol at host mw)
    real(kind_phys), intent(in)  :: a_poa_in(ntot_amode, ntot_poaspec)    ! initial poa aerosol mixrat (mol/mol at host mw)
    real(kind_phys), intent(in)  :: xferrate_in(ntot_amode, ntot_soaspec) ! gas-aerosol mass transfer rate (1/s)
    real(kind_phys), intent(out) :: g_soa_tend(ntot_soaspec)             ! soa gas mixrat tendency (mol/mol/s at host mw)
    real(kind_phys), intent(out) :: a_soa_tend(ntot_amode, ntot_soaspec)  ! soa aerosol mixrat tendency (mol/mol/s at host mw)
!     integer,  intent(in)  :: idiagss
    ...
    real(kind_phys) :: rgas
    real(kind_phys), parameter :: a_min1 = 1.0e-20_kind_phys
    real(kind_phys), parameter :: g_min1 = 1.0e-20_kind_phys
    real(kind_phys), parameter :: alpha = 0.05_kind_phys     ! parameter used in calc of time step
    real(kind_phys), parameter :: dtsub_fixed = -1.0_kind_phys  ! fixed sub-step for time integration (s)

    real(kind_phys) :: a_ooa_sum_tmp(ntot_soamode)          ! total ooa (=soa+opoa) in a mode
    real(kind_phys) :: a_opoa(ntot_soamode)                 ! oxidized-poa aerosol mixrat (mol/mol at actual mw)
    real(kind_phys) :: a_soa(ntot_soamode, ntot_soaspec)     ! soa aerosol mixrat (mol/mol at actual mw)
    real(kind_phys) :: a_soa_tmp(ntot_soamode, ntot_soaspec) ! temporary soa aerosol mixrat (mol/mol)
    real(kind_phys) :: beta(ntot_soamode, ntot_soaspec)      ! dtcur*xferrate
    real(kind_phys) :: delh_vap_soa(ntot_soaspec)           ! delh_vap_soa = heat of vaporization for gas soa (J/mol)
    real(kind_phys) :: del_g_soa_tmp(ntot_soaspec)
    real(kind_phys) :: dtcur                                ! current time step (s)
    ...
    real(kind_phys) :: phi(ntot_soamode, ntot_soaspec)       ! "relative driving force"
    real(kind_phys) :: p0_soa(ntot_soaspec)                 ! soa gas equilib vapor presssure (atm)
    real(kind_phys) :: p0_soa_298(ntot_soaspec)             ! p0_soa_298 = soa gas equilib vapor presssure (atm) at 298 k
    real(kind_phys) :: sat(ntot_soamode, ntot_soaspec)       ! sat(m,ll) = g0_soa(ll)/a_ooa_sum_tmp(m) = g_star(m,ll)/a_soa(m,ll)
    !    used by the numerical integration scheme -- it is not a saturation rato!
    real(kind_phys) :: tcur                                 ! current integration time (from 0 s)
    real(kind_phys) :: tmpa, tmpb, tmpf
    real(kind_phys) :: tot_soa(ntot_soaspec)                ! g_soa + sum( a_soa(:) )
    real(kind_phys) :: xferrate(ntot_amode, ntot_soaspec)    ! gas-aerosol mass transfer rate (1/s)

    rgas = rgas_m

    ! Changed by Manish Shrivastava
    opoa_frac(:) = 0.0_kind_phys !POA does not form solution with SOA for all runs; set opoa_frac=0.0_kind_phys  by Manish Shrivastava
    mw_poa(:) = 250.0_kind_phys
    mw_soa(:) = 250.0_kind_phys
    ...
    if (ntot_soaspec == 1) then
      ...
      p0_soa_298(3) = 9.7831E-11_kind_phys !soaff2 C*=1.0ug/m3
      p0_soa_298(4) = 9.7831E-10_kind_phys !soaff3 C*=10.0ug/m3
      p0_soa_298(5) = 9.7831E-9_kind_phys  !soaff4 C*=100.0ug/m3

      delh_vap_soa(1) = 153.0e3_kind_phys
      delh_vap_soa(2) = 142.0e3_kind_phys
      ...
      p0_soa_298(4) = 9.7831E-10_kind_phys !soaff3 C*=10.0ug/m3
      p0_soa_298(5) = 9.7831E-9_kind_phys  !soaff4 C*=100.0ug/m3
      p0_soa_298(6) = 9.7831E-13_kind_phys !soabb0 C*=0.01ug/m3
      p0_soa_298(7) = 9.7831E-12_kind_phys !soabb1 C*=0.10ug/m3
      p0_soa_298(8) = 9.7831E-11_kind_phys !soabb2 C*=1.0ug/m3
      p0_soa_298(9) = 9.7831E-10_kind_phys !soabb3 C*=10.0ug/m3
      p0_soa_298(10) = 9.7831E-9_kind_phys  !soabb4 C*=100.0ug/m3
      p0_soa_298(11) = 9.7831E-13_kind_phys !soabg0 C*=0.01ug/m3
      p0_soa_298(12) = 9.7831E-12_kind_phys !soabg1 C*=0.1ug/m3
      p0_soa_298(13) = 9.7831E-11_kind_phys !soabg2 C*=1.0ug/m3
      p0_soa_298(14) = 9.7831E-10_kind_phys !soabg3 C*=10.0ug/m3
      p0_soa_298(15) = 9.7831E-9_kind_phys  !soabg4 C*=100.0ug/m3
      ...
      delh_vap_soa(4) = 120.0e3_kind_phys
      delh_vap_soa(5) = 109.0e3_kind_phys
      delh_vap_soa(6) = 153.0e3_kind_phys
      delh_vap_soa(7) = 142.0e3_kind_phys
      delh_vap_soa(8) = 131.0e3_kind_phys
      delh_vap_soa(9) = 120.0e3_kind_phys
      delh_vap_soa(10) = 109.0e3_kind_phys
      delh_vap_soa(11) = 153.0e3_kind_phys
      delh_vap_soa(12) = 142.0e3_kind_phys
