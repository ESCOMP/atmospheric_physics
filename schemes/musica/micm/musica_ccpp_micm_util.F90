...
contains
  ...
  subroutine update_micm_state(state, states_updated, temperature, pressure, &
    ...
    integer,                             intent(in)    :: states_updated         ! number of states already updated
    ...
    integer :: i_cell, i_var, state_offset, n_cells
    real(kind_phys), pointer :: temperature_1D(:), pressure_1D(:), air_density_1D(:), &
                                species_1D(:), params_1D(:)

    ! offset data in the grid cell dimension
    n_cells = state%number_of_grid_cells
    state_offset = states_updated * n_cells

    ! Update environmental conditions
    ! collapse 2D arrays to 1D
    ! (column, layer) -> (column*layer)
    temperature_1D(1:n_cells) => temperature(:,:)
    pressure_1D(1:n_cells) => pressure(:,:)
    air_density_1D(1:n_cells) => dry_air_density(:,:)
    do i_cell = 1, n_cells
      state%conditions(i_cell)%temperature = temperature_1D(i_cell + state_offset)
      state%conditions(i_cell)%pressure    = pressure_1D(i_cell + state_offset)
      state%conditions(i_cell)%air_density = air_density_1D(i_cell + state_offset)
    end do
    ...
    do i_var = 1, state%number_of_species
      species_1D(1:n_cells) => mixing_ratios(:,:,micm_indices_constituent_props(i_var))
      do i_cell = 1, n_cells
        state%concentrations( 1 + ( i_cell - 1 ) * cell_stride + ( i_var - 1 ) * var_stride ) = &
          species_1D(i_cell + state_offset) * state%conditions(i_cell)%air_density &
    ...
    do i_var = 1, state%number_of_rate_parameters
      params_1D(1:n_cells) => rate_parameters(:,:,i_var)
      do i_cell = 1, n_cells
        state%rate_parameters( 1 + ( i_cell - 1 ) * cell_stride + ( i_var - 1 ) * var_stride ) = &
          params_1D(i_cell + state_offset)
      end do
    end do
    end associate
  ...
  !! species.
  subroutine extract_mixing_ratios_from_state(state, states_updated, mixing_ratios)
    ...
    do i_var = 1, state%number_of_species
      species_1D(1:n_cells) => mixing_ratios(:,:,micm_indices_constituent_props(i_var))
      do i_cell = 1, n_cells
        species_1D(i_cell + state_offset) = &
          state%concentrations( 1 + ( i_cell - 1 ) * cell_stride + ( i_var - 1 ) * var_stride ) &
