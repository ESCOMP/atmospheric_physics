...
contains
  ...
  subroutine test_reshape()
    ...
    integer, parameter :: NUM_GRID_CELLS = NUM_COLUMNS * NUM_LAYERS
    real,    parameter :: ABS_ERROR = 1e-7
    real(c_double)     :: expected_conditions(NUM_GRID_CELLS) = [100.0, 200.0, 300.0, 400.0]
    real(c_double)     :: expected_constituents(NUM_GRID_CELLS*NUM_SPECIES) = &
      [0.1, 0.2, 0.3, 0.4, 0.21, 0.22, 0.23, 0.24, 0.41, 0.42, 0.43, 0.44, 0.31, 0.32, 0.33, 0.34]
    ...
    do i_elem = 1, NUM_GRID_CELLS
      ASSERT(micm_temperature(i_elem) == expected_conditions(i_elem))
      ASSERT(micm_pressure(i_elem) == expected_conditions(i_elem))
      ASSERT(micm_dry_air_density(i_elem) == expected_conditions(i_elem))
    end do

    do i_elem = 1, size(micm_constituents)
      ASSERT_NEAR(micm_constituents(i_elem), expected_constituents(i_elem), ABS_ERROR)
    end do
    ...
    do i_layer = 1, NUM_LAYERS
      do i_column = 1, NUM_COLUMNS
        do i_elem = 1, NUM_SPECIES
          ASSERT_NEAR(constituents(i_column, i_layer, i_elem), expected_constituents(i_arr), ABS_ERROR)
          i_arr = i_arr + 1
        end do
      end do
    end do
